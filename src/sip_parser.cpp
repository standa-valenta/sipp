...
void getContactURL (const char* msg, char* buf)
{
     ...
     */
    const char* next_req_url = internal_find_header(msg, "Contact", "m", true);
    const char* start = strchr(next_req_url, '<');
    const char* end = strrchr(next_req_url, '>');
    if (start && end)
    {
        strncpy(buf, start + 1, (end - start - 1));
    } else {
        const char* end_of_header = internal_hdrend(next_req_url);
        strncpy(buf, next_req_url, (end_of_header - next_req_url));
    }
}
...
TEST(Parser, get_req_uri) {
    char buf[255];
    getRequestURI("INVITE sip:example.com SIP/2.0\r\nContact: abc@def\r\n\r\n", buf);
    EXPECT_STREQ("sip:example.com", buf);
}

TEST(Parser, get_contact) {
    char buf[255];
    getContactURL("...\r\nContact: abc@def\r\n\r\n", buf);
    EXPECT_STREQ("abc@def", buf);
}

TEST(Parser, get_contact_2) {
    char buf[255];
    getContactURL("...\r\nContact: <abc@def>\r\n\r\n", buf);
    EXPECT_STREQ("abc@def", buf);
}
